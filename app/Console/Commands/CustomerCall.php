...
class CustomerCall extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'customer:call';
    ...
    public function handle()
    {
        ...
        DB::table('subscribe_users')->where('optin','subscribed')->orderBy('created_at','desc')->chunk(20, function($data) use($mobin)
        {
            ...
            foreach ($data as $key=>$val)
            {
                ...
                $customercallreminder = strpos($tempData->message, 'for your interest, your request has been registered.') !== FALSE ? true : false;
                ...
                if( strtotime($temp) < strtotime($last20Hours) && strtotime($temp) > strtotime($last23Hours)){
                    ...
                    foreach ($forms as $key=>$form) {
                        if ($env == 'production' || in_array($form->mobile, $mobin) && $customercallreminder && $tempData->count < 2 ){
