...
class WhatsappReminderAfterSevenDays extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'reminder:seven-days';
     ...
     * @var string
     */
    protected $description = 'Whatsapp reminder after seven days';
    ...
    public function handle()
    {
        ...
        {
            $mobileArray = array();
            $last20Hours = Carbon::now('Asia/Kolkata')->subHours(165)->format('Y-m-d H:i:s');
            $last23Hours = Carbon::now('Asia/Kolkata')->subHours(168)->format('Y-m-d H:i:s');
            ...
            foreach ($data as $key=>$val)
            {
                ...
                // $remindertwodays = strpos($tempData->message, 'The EMI details of your plan are:') !== FALSE ? true : false;
                ...
                if( strtotime($temp) < strtotime($last20Hours) && strtotime($temp) > strtotime($last23Hours)){
                    ...
                    foreach ($forms as $key=>$form) {
                        if ($env == 'production' || in_array($form->mobile, $mobin) && $tempData->count < 2 && ($tempData->message == 'financial_solution')){
                            $this->sendReminderMessage($form->mobile, $form->model_crm_name);
                        }
                    }
                }
            }           
    ...
    private function sendReminderMessage($mobile, $model) {
