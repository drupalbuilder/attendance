...
class whatsappCallingVerifyPerDay extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'whatsapp:verifyPerDay';
     ...
     * @var string
     */
    protected $description = 'Whatsapp number verify and update per day';
    ...
    public function handle()
    {
        $env = Config('app.env');
        try{
        $subscribeUser = SubscribeUser::where('optenabled', 'no')->where('count', '>' ,'14')->get();
        $contacts = array();
        ...
        if (isset($res['reason'])) {
            ...
            LogHelper::log('scheduler:whatsappCallingVerifyPerDay', $res['reason'], 'error');
        } else {
            
            LogHelper::log('scheduler:whatsappCallingVerifyPerDay', 'successfully run', 'info');
        ...
        LogHelper::log('scheduler:whatsappCallingVerifyPerDay', $e->getMessage(), 'exception');
